...
begin
   ...
   PADTS.dtstamp; ret_open        := PAAPPDEV.GetA_open_parport(port, fd_attrib);
   fd := ret_open; 
   PADTS.dtstamp; ret_claim       := PAAPPDEV.GetA_ppclaim_parport (fd);
   PADTS.dtstamp; ret_setmode     := PAAPPDEV.GetA_ppsetmode_parport (fd, modetoset);
   PADTS.dtstamp; ret_getmode     := PAAPPDEV.GetA_ppgetmode_current (fd);
   PADTS.dtstamp; ret_direction   := PAAPPDEV.GetA_ppdatadir_dataport (fd, datadirection);
   -- WRITE STREAM DATA TO PARALLEL PORT DATA REGISTER
   PADTS.dtstamp; ret_writestream := PAAPPDEV.GetA_streamdata_writedataregister (fd, stream_datatowrite);
   PADTS.dtstamp; ret_release     := PAAPPDEV.GetA_pprelease_parport; 
   PADTS.dtstamp; ret_close       := PAAPPDEV.GetA_close_parport(fd); 
